...
def cadastro():
    if request.method == 'POST':
        codigo = gerar_codigo(Cliente, prefixo='JSP')
        cliente = Cliente(
            codigo=codigo,
...
def cadastrar_produto(id=None):
    ...
    if request.method == 'POST':
        ...
        if not data.get('codigo'):
            data['codigo'] = gerar_codigo(Produto, prefixo='JSP')
        try:
...
def cadastrar_servico(id=None):
    ...
    if request.method == 'POST':
        data = request.form.to_dict()
        if not data.get('codigo'):
            data['codigo'] = gerar_codigo(Servico, prefixo='JSP')
...
def cadastrar_fornecedor(id=None):
    ...
    if request.method == 'POST':
        data = request.form.to_dict()
        if not data.get('codigo'):
            data['codigo'] = gerar_codigo(Fornecedor, prefixo='JSP')
        if fornecedor:
            for key, value in data.items():
...
def cadastro_ordem_servico(id=None):
    ...
    if request.method == 'POST':
        data = request.form.to_dict()
        if not data.get('codigo'):
            data['codigo'] = gerar_codigo(OrdemServico, prefixo='JSP')
